...
export default function RegisterForm() {
  ...
  const fieldsByStep: (keyof RegisterInput)[][] = [
    ['firstName', 'lastName', 'username', 'email', 'countryCode', 'phoneNumber', 'password', 'confirmPassword'],
  ...
  const onSubmit = async (data: RegisterInput) => {
    ...
    try {
      ...
      await registerUser({ ...data, phoneNumber: fullPhoneNumber })
        ...
        <AnimatePresence mode="wait">
            ...
            >
              ...
              <div>
                ...
                <p className="mt-1 text-xs text-brand-ink/40 dark:text-white/40">
                  Enter your number without the leading 0 — e.g. for {selectedCountryCode} 712345678
                </p>
          ...
          {step === 1 && (
            ...
            >
              ...
              <div>
                <label className="text-xs font-semibold text-brand-ink/60 dark:text-white/60">Country</label>
                <select className="input-field mt-1" {...register('country')}>
                  {COUNTRY_CODES.map((c) => (
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-xs font-semibold text-brand-ink/60 dark:text-white/60">County/Region</label>
                  <input className="input-field mt-1" placeholder="e.g. Nairobi, California" {...register('county')} />
                  {errors.county && <p className="mt-1 text-xs text-red-500">{errors.county.message}</p>}
                </div>
                <div>
                  <label className="text-xs font-semibold text-brand-ink/60 dark:text-white/60">Town/City</label>
