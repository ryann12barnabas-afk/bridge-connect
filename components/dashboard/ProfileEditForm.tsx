...
import { HiOutlineCamera, HiOutlineXMark, HiOutlinePlus } from 'react-icons/hi2'
...
export default function ProfileEditForm() {
  ...
  const handleGalleryAdd = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setPhotos(updated)
      await setDoc(doc(db, 'profiles', user.uid), { photos: updated, updatedAt: new Date().toISOString() }, { merge: true })
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          >
            <HiOutlineCamera /> {uploading ? 'Uploading…' : 'Change main photo'}
          </button>
      ...
      <div>
        <label className="text-xs font-semibold text-brand-ink/60 dark:text-white/60">
        ...
        <div className="mt-2 grid grid-cols-3 gap-2">
          ...
          {photos.length < MAX_PHOTOS && (
            <button
              ...
              disabled={uploading}
              className="grid aspect-square place-items-center rounded-xl border-2 border-dashed border-gray-300 text-gray-400 dark:border-white/20"
            >
              <HiOutlinePlus className="h-6 w-6" />
            </button>
          )}
        </div>
        <input ref={galleryInputRef} type="file" accept="image/*" hidden onChange={handleGalleryAdd} />
      </div>
