...
export default function MatchesPage() {
  ...
  useEffect(() => {
    if (!user) return
    getAllBlockedUids(user.uid).then(setBlockedUids)
  }, [user])
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, async (snap) => {
      ...
      const notBlocked = (m: Match) => {
        const otherUid = m.users.find((u) => u !== user.uid)!
        return !blockedUids.includes(otherUid)
      }

      const pendingForMe = all.filter((m) => m.status === 'pending' && m.initiatedBy !== user.uid && notBlocked(m))
      const activeOnes = all.filter((m) => m.status === 'accepted' && notBlocked(m))
    ...
    <div className="mx-auto max-w-2xl space-y-10">
          ...
          <div className="space-y-3">
            {incoming.map((m) => (
              <div key={m.id} className="card flex items-center gap-4 p-4">
                <button onClick={() => setSelected(m)} className="h-12 w-12 shrink-0 rounded-full bg-bridge-gradient" aria-label="View profile" />
      ...
      <div>
        ...
        {accepted.length === 0 ? (
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {accepted.map((m) => (
              <button key={m.id} onClick={() => setSelected(m)} className="card flex items-center gap-3 p-4 text-left">
      ...
      {user && (
        <MatchProfileModal
          ...
          otherProfile={selected?.otherProfile}
