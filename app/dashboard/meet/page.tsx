...
function SwipeCard({
  ...
  const photos = [candidate.profile.photoURL, ...(candidate.profile.photos || [])].filter(Boolean)
  const hasPhotos = photos.length > 0
  const embedUrl = candidate.profile.spotifyTrackUrl ? getSpotifyEmbedUrl(candidate.profile.spotifyTrackUrl) : null
  ...
  return (
    <motion.div
      className="card absolute inset-0 overflow-y-auto"
    ...
    >
      <div className="relative aspect-[3/4] w-full overflow-hidden bg-gradient-to-br from-brand-blueLight to-brand-purple">
        {hasPhotos && (
          // eslint-disable-next-line @next/next/no-img-element
          <img src={photos[photoIndex]} alt={candidate.user.firstName} className="h-full w-full object-cover" />
        )}
        ...
        {isTop && photos.length > 1 && (
          <>
            <button onClick={goPrev} className="absolute left-0 top-0 h-full w-1/2" aria-label="Previous photo" />
            <button onClick={goNext} className="absolute right-0 top-0 h-full w-1/2" aria-label="Next photo" />
          </>
        )}
        ...
        {isTop && (
          <>
            <motion.span
              ...
              className="absolute left-4 top-8 rotate-[-15deg] rounded-lg border-4 border-green-400 px-3 py-1 text-lg font-extrabold text-green-400"
            >
              LIKE
            </motion.span>
            <motion.span
              style={{ opacity: passOpacity }}
              className="absolute right-4 top-8 rotate-[15deg] rounded-lg border-4 border-red-400 px-3 py-1 text-lg font-extrabold text-red-400"
            >
              PASS
            </motion.span>
          </>
        ...
        <span className="absolute bottom-3 right-3 rounded-full bg-black/40 px-3 py-1 text-xs font-semibold text-white backdrop-blur">
      ...
      <div className="p-5">
        ...
        {candidate.profile.interests.length > 0 && (
          <div className="mt-3 flex flex-wrap gap-1.5">
            {candidate.profile.interests.map((i) => (
