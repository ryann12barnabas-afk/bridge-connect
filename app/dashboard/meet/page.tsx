...
import { findMeetCandidatesBatch, consumeFreeMeet } from '@/lib/meet'
...
export default function MeetPage() {
  ...
  const canLike = isPremium || freeMeetsLeft > 0
  ...
  const handleSwipe = async (direction: 'left' | 'right', candidate: Candidate) => {
    ...
    if (!canLike) {
      toast.error('You have used all your free meetings.')
    ...
    try {
      ...
      if (!isPremium) await consumeFreeMeet(user.uid)
    ...
    <div className="mx-auto max-w-md">
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-brand-ink/60 dark:text-white/60">
            {isPremium ? 'Unlimited likes with Premium' : `${freeMeetsLeft} free likes remaining`}
          </p>
        </div>
        <Link
      ...
      {!canLike && (
        <div className="card mt-6 p-5 text-center">
          ...
          <p className="mt-2 text-sm font-semibold">You have used all your free meetings.</p>
